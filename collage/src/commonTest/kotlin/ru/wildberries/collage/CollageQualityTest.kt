...
private data class CaseCfg(val tag: String, val collageImages: List<CollageImage>)

private fun totalHeightError(geom: CollageLayout): String? {
...
private fun rowVerticalSquashError(
    ...
    val vCnt = row.tiles.count {
        val p = collageImageById[it.imageId] ?: error("can't find element by ${it.imageId}")
...
private fun checkGeometry(
    ...
    val paddings = cfg.spacing
    val minItemWidth = cfg.minItemWidth
    val photoById = case.collageImages.associateBy { it.imageId }
...
private fun runSuite(
    ...
    for (c in cases) {
        ...
        if (errs.isNotEmpty()) {
            ...
            println("OK $suiteName / ${c.tag}  photos=${c.collageImages.size}  rows=${geom.rows.size}  H=${fmt(geom.height)}")
        }
    }
    ...
    fun property_randomized_1_to_12() {
        ...
        for (n in 1..12) {
            for (seed in 0L..7L) {
                ...
                val renum = photos.mapIndexed { i, p -> CollageImage(idBase + i, p.width, p.height) }
                idBase += n + 1
