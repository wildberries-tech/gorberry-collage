...
/**
 * Open addressing cache for fixed range row plans within a single layout pass
 *
 * Row boxes are stored in primitive pools to reduce allocations during dynamic
 * programming search and refinement
 */
internal class RowPlanCache {
    ...
    fun boxesAsList(slot: Int): List<RectF> {
        if (slot == NO_SLOT) return emptyList()
