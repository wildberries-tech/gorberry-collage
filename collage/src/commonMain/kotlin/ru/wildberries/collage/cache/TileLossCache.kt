...
/**
 * Open-addressing cache for tile fit decisions within a single layout pass.
 *
 * clear() advances an epoch instead of wiping arrays, which avoids repeated fills
 * while keeping stale entries invisible to the current pass
 */
internal class TileLossCache {
    ...
    fun get(photoKey: Int, boxKeyWH: Int): TileLossDecision? {
    ...
    fun put(photoKey: Int, boxKeyWH: Int, tileLossDecision: TileLossDecision) {
        ensureForInsert()
        ...
        while (true) {
            ...
            while (probes < tableSize) {
                ...
                if (!occupied) {
                    ...
                    coverLossTable[pos] = tileLossDecision.cover
                    containLossTable[pos] = tileLossDecision.contain
                    cropRatioTable[pos] = tileLossDecision.crop
                    useCoverTable[pos] = tileLossDecision.useCover
                    size++
                    return
                }
