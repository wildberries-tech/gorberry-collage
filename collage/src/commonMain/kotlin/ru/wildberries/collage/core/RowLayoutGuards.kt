...
/**
 * Rejects row candidates that would produce unreadable tiles or unacceptable content loss
 */
internal object RowLayoutGuards {
    ...
    fun hasInvalidSelectedTileContent(
        ...
        while (tileIndex < boxes.size) {
            ...
            if (useCover) {
                val rawCropRatio = TileFitLossModel.coverCropRatio(photo, box)
    ...
    private fun computeContainContentSize(
        ...
        val safePhotoWidth = max(collageImage.width, 1e-6f)
        val safePhotoHeight = max(collageImage.height, 1e-6f)
