...
internal object MathUtil {
    ...
    /**
     * Quantizes width and height into 16-bit components and packs them into a 32-bit key
     */
    ...
    private fun freeCropForWideImage(
        ...
        return when {
            // Do not discount crop that makes an already extreme aspect ratio worse
            boxAspectRatio >= imageAspectRatio -> 0f

            // The box aspect moves the image toward the free crop aspect limit
            boxAspectRatio >= freeCropAspectLimit -> rawCropRatio

            // Only the crop up to the free crop aspect limit is discounted
    ...
    class PowerLookupTable(
        ...
        /**
         * Approximates pow(value, exponent) using bilinear interpolation over a 2D lookup table
         */
        fun power(
            valueInput: Float,
            exponentInput: Float,
