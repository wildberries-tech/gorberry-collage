...
object TestKit {
    ...
    fun p(id: Int, w: Float, h: Float) = Photo(id, w, h)

    fun gridSquare(n: Int, size: Float) =
        (0 until n).map { i -> p(i, size, size) }

    fun mixedSample1(): List<Photo> = listOf(
        p(0, 1200f, 800f),
        p(1, 900f, 1600f),
        p(2, 1000f, 1000f),
        p(3, 1600f, 900f),
    ...
    fun alternatingHV(n: Int, base: Float = 1000f): List<Photo> =
        (0 until n).map { i ->
            if (i % 2 == 0) p(i, base, base * 0.625f) else p(i, base * 0.56f, base)
        }

    fun nearMinWidthVerticals(count: Int, base: Float = 1000f, targetAR: Float = 0.5f): List<Photo> =
        (0 until count).map { i -> p(i, base * targetAR, base) }

    fun adversarialWideAndThin(nThin: Int = 3, base: Float = 1000f): List<Photo> {
    ...
    fun quasiSquares(n: Int, base: Float = 1000f, delta: Float = 0.08f): List<Photo> =
        (0 until n).map { i ->
    ...
    fun staircaseBlocks(
        ...
        var id = 0
        val out = ArrayList<Photo>(repeats * (blockH + blockV + blockS))
        repeat(repeats) {
    ...
    fun longMixed(n: Int = 30, seed: Long = 9L): List<Photo> = randomMixed(n, seed)
    ...
    fun allUltraH(n: Int, base: Float = 1000f): List<Photo> {
        val pool = floatArrayOf(2.8f, 3.2f, 3.8f, 4.5f, 5.0f, 5.5f)
        return (0 until n).map { i -> Photo(i, base * pool[i % pool.size], base) }
    }

    fun allUltraV(n: Int, base: Float = 1000f): List<Photo> {
        val pool = floatArrayOf(1 / 2.8f, 1 / 3.2f, 1 / 3.8f, 1 / 4.5f, 1 / 5.0f, 1 / 5.5f)
        return (0 until n).map { i -> Photo(i, base * pool[i % pool.size], base) }
    }

    fun ultraSaw(nPairs: Int, base: Float = 1000f): List<Photo> {
        val out = ArrayList<Photo>(2 * nPairs)
        var id = 0
        ...
        repeat(nPairs) {
            ...
            out += Photo(id++, base * w, base)
            out += Photo(id++, base * t, base)
        }
        return out
    }
}
