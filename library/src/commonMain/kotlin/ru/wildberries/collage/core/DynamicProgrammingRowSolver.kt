...
internal data class DynamicProgrammingParams(
    ...
    val rowLenPrior: RowLengthPriority = RowLengthPriority(),
)

internal data class RowSolve(
    val cost: Double,
...
internal fun betterSolve(candidate: RowSolve, current: RowSolve?): Boolean {
...
internal fun singletonCount(solve: RowSolve): Int =
    solve.ranges.count { it.first == it.last }

internal fun hasAnySingleton(solve: RowSolve): Boolean =
    singletonCount(solve) > 0
...
/** DP split of tiles into rows */
internal class DynamicProgrammingRowSolver(
    private val params: DynamicProgrammingParams,
    private val tileScorer: TileScorer,
    ...
    private fun buildPlanAtHeight(
        ...
        val cached = planCache.get(i, j, q)
        if (cached >= 0) {
            return Plan(
                boxes = planCache.boxesAsList(cached),
                loss = planCache.rowLossBySlot[cached].toDouble(),
                height = planCache.rowHeightBySlot[cached]
            )
        }
        ...
        return Plan(planCache.boxesAsList(slot), planCache.rowLossBySlot[slot].toDouble(), planCache.rowHeightBySlot[slot])
    }
    ...
    private inner class Session(
        ...
        val aspectReal: FloatArray = FloatArray(n) { i -> MathUtil.aspect(photos[i].width, photos[i].height) }
        ...
        private val memo: Array<Array<RowSolve?>> = Array(n + 1) { arrayOfNulls(targetRows + n + 2) }
        ...
        fun totalCostOrNull(
            ...
            penaltyRaw: Double,
            tailCost: Double,
        ): Double? = this@DynamicProgrammingRowSolver.computeTotalCostOrNull(
        ...
        fun solve(): RowSolve = bestFrom(0, 0)

        fun bestFrom(startIndex: Int, rowIndex: Int): RowSolve {
            if (startIndex == n) {
            ...
            while (length <= maxLenHere) {
                ...
                if (candidate != null && betterSolve(candidate, best)) {
                    best = candidate
                }
                length++
            }

            val res = best ?: RowSolve(
                cost = Double.POSITIVE_INFINITY,
                lossScore = Double.POSITIVE_INFINITY,
                ranges = emptyList(),
                rowHeights = emptyList(),
    ...
    private fun evaluateLengthCandidate(
        ...
        for (candidateHeight in candidateHeights) {
            ...
            if (solve != null && betterSolve(solve, best)) {
                best = solve
            }
        }
