...
internal data class RowLayoutContext(
    ...
    val tileScorer: TileScorer,
) {

    val length: Int get() = endIndexInclusive - startIndexInclusive + 1
    fun photoAt(offset: Int): Photo = photos[startIndexInclusive + offset]
}
...
internal class RowPlanner {
    ...
    /** Returns feasible height range for a row */
    ...
    private fun normalizedCoverCropForGuard(
        ...
        val imageAspectRatio = MathUtil.aspect(photo.width, photo.height)
    ...
    /** Row loss with narrow container guard */
    ...
    private fun tileLossForRowScoring(
        ...
        val decision = context.tileScorer.decide(photo, box)
    ...
    /** Snap widths to target span, applying per aspect floors and min frac guards */
    ...
    /** Raise row height if any tile would suffer extreme cover/crop */
